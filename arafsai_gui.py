...
gif_path = os.path.join(script_dir, 'arafsaigui', 'im.gif')
pico_gif_path = os.path.join(script_dir, 'arafsaigui', 'picogram.gif')
...
def main():
    ...
    while running:
        ...
        # Render arafsai GIF
        arafsai_frame = frame_surfaces[frame_idx]
        arafsai_scaled = pygame.transform.scale(arafsai_frame, (scaled_width, scaled_height)).convert_alpha()
        
        # Cyan tint effect
        arafsai_tint = pygame.Surface((scaled_width, scaled_height), pygame.SRCALPHA)
        arafsai_tint.fill(CYAN + (128,))
        arafsai_scaled.blit(arafsai_tint, (0, 0), special_flags=pygame.BLEND_RGBA_MULT)

        # Center arafsai
        arafsai_rect = arafsai_scaled.get_rect(center=(screen.get_width() // 2, screen.get_height() // 2))
        screen.blit(arafsai_scaled, arafsai_rect)
